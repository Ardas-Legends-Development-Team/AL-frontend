...
export class RpCharApiClient extends ApiClient {
    ...
    public static loadRpCharsByNames(names: string[]): Promise<RoleplayCharacter[]> {
        const rpCharStore = useRpCharStore();
        return new Promise((resolve) => {

            const loadedCharNames: string[] = rpCharStore.rpChars.map(rp => rp.rpChar.name);
            ...
            names.forEach(name => {
                ...
                if(index !== -1) {
                    alreadyLoadedChars.push(rpCharStore.rpChars.at(index) as RoleplayCharacter)
                }
                else
                 charsToFetch.push(name);
            })
